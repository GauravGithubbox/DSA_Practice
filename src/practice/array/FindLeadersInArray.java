/*Given an practice.array A of positive integers. Your task is to find the leaders in the practice.array. An element of the practice.array is a leader if it is greater than all the elements to its right side or if it is equal to the maximum element on its right side. The rightmost element is always a leader.
...
public class FindLeadersInArray {
    static ArrayList<Integer> leaders(int A[], int n) {
        ...
        // Traverse the practice.array from the second last element to the first element
        for (int i = n - 2; i >= 0; i--) {
            if (A[i] >= maxFromRight) {
                maxFromRight = A[i];
                leaders.add(maxFromRight);
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Leaders in the practice.array: " + result);
    }
}
